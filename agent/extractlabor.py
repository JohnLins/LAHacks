...
def extract_human_tasks_from_prompt(prompt: str) -> list[dict[str, Any]]:
    ...
    """
    api_key = os.getenv("ASI1_API_KEY") or os.getenv("FETCH_KEY")
    if not api_key:
        debug = os.getenv("EXTRACTLABOR_DEBUG", "").lower() in {"1", "true", "yes"}
        if debug:
            print(
                "[extractlabor] Missing ASI1_API_KEY (or FETCH_KEY); returning []"
            )
    ...
    client = OpenAI(
        base_url=os.getenv("ASI_BASE_URL", "https://api.asi1.ai/v1"),
        api_key=api_key,
    )
    model = os.environ.get("ASI_MODEL", "asi1-mini")
    ...
    user = system + f"\n\nPrompt:\n{prompt}\n\nReturn JSON now:"
    if debug:
        print(f"[extractlabor] Using model={model} base_url={os.getenv('ASI_BASE_URL', 'https://api.asi1.ai/v1')}")
    ...
    try:
        ...
        if debug:
            print(
                f"[extractlabor] ASI inference error using model={model}, key={_mask_secret(api_key)}: {e}"
            )
        return []
